...
def get_tokenizer():
    with open(PATH + '/assets/etc/vocab.txt') as f:
        words = [i[:-1] for i in f.readlines()]
    csv = read_csv(PATH + '/assets/csv/names.csv')
    train = csv['text']
    token = text.Tokenizer(num_words=None, oov_token='ауф')
...
Labels2Commands = {0: open_explorer, 1: shutdown, 2: open_browser,
                   3: symbol, 4: weather, 5: open_terminal, 6: new_name}


if __name__ == '__main__':
    new_name('меня зовут ваня')
